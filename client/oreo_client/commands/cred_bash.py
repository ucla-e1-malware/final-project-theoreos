...
import shutil
import os
# cred_bash e1-target.local ftp usernames.txt passwords.txt
class PasswordBash(Command):
    """
    Usage: cred_bash <target> <service> <user_list> <pass_list>
    Example: cred_bash e1-target.local ftp usernames.txt passwords.txt
    """
    ...
    def do_command(self, lines: str, *args):
        ...
        if len(parts) < 4:
            ...
            print("Example: cred_bash e1-target.local ftp my_users.txt my_pass.txt")
            return
        ...
        try:
            # Stream output directly to the terminal for live feedback
            result = subprocess.run(audit_command, check=False)
            
            if result.returncode == 0:
                print("\n[+] Audit completed successfully. Check the terminal output above for credentials.")
            else:
